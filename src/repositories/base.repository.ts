...
import { PageDto } from 'src/common/pagination/page-common.dto';
...
export abstract class IBaseRepository<T> extends Repository<T> {
    ...
    public async pagination(
        ...
        try {
            ...
            console.log('Count of non-deleted records:', itemCount);
            const pageMetaDto = new PageMetaDto(pageOptionsDto, itemCount);
            return new PageDto(plainToInstance(dto, entities), pageMetaDto);
        } catch (error) {
            console.error('Error counting non-deleted records:', error);  
            throw error;
        }
    }
